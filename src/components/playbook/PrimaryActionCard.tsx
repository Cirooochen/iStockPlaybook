...
export function PrimaryActionCard({ zone, currentShares, recommendedShares, onAct }: Props) {
  ...
  return (
    <div className="bg-white rounded-lg border border-stone-200 p-6 mb-6">
      ...
      <div className="flex items-start justify-between gap-3 mb-2">
        <div className="flex items-center gap-3">
          <span className="relative inline-flex shrink-0">
            <ActionIcon className="w-8 h-8 p-1.5 rounded-full border border-stone-300 text-stone-600" aria-hidden="true" />
            {badge && (
          ...
          <h2 className="text-xl font-bold text-stone-900 tracking-tight">{zone.title}</h2>
        </div>
        <span
      ...
      <p className="text-sm text-stone-600 leading-relaxed max-w-2xl mb-4">{zone.summary}</p>
      ...
      {canAct ? (
        ...
        zone.primaryTrigger && (
          <p className="text-sm text-stone-500 italic">{zone.primaryTrigger}</p>
        )
      )}

      <div className="flex gap-4 mt-4 pt-4 border-t border-stone-100 text-sm text-stone-500">
        <span>
